...
export async function GET(request: Request) {
  try {
    ...
    if (!matchedFile && normalizedTitle) {
      matchedFile =
        files.find(
          (file) =>
            normalize(file) === normalizedTitle
        ) ||
        files.find(
          (file) =>
            normalize(file).includes(normalizedTitle)
        ) ||
        null;
    }
    ...
    console.error("Audio match error:", error);
